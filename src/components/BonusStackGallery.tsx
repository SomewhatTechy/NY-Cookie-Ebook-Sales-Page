...
const BonusStackGallery: React.FC = () => {
  ...
  // Supports /public/bonuses/gallery/{en|es|pt}
  // If a PT asset is missing, the <img onError> fallback loads EN.
  const langFolder = language === 'pt' ? 'pt' : language
  const img = (filename: string) => `/bonuses/gallery/${langFolder}/${filename}`
  const fallbackImg = (filename: string) => `/bonuses/gallery/en/${filename}`
